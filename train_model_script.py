...
def train_preprocessing(training_filepath):
    ...
    for col in df.columns:
        if "creatinine_date" in col:
            df[col] = pd.to_datetime(
                df[col], format="%m/%d/%y %H:%M:%S", errors="coerce"
    ...
    X = df.iloc[
        :, ::-1
    ]  # reverse so that most recent test results appear first (match with test processing)
...
def training(training_filepath):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_X, batch_y in dataloader:
            ...
            predictions = (
                outputs > 0.3
            ).int()  # Lower than usual because we care more about FN
        ...
        f3_score_training = fbeta_score(
            all_labels, all_predictions, beta=3, zero_division=1
        )
        print(
            f"Epoch [{epoch+1}/{num_epochs}], Loss: {loss.item():.4f}, F3 Score: {f3_score_training:.4f}"
        )
