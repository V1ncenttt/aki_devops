...
------
Example:
    listener = MllpListener("127.0.0.1:5000", parser, data_operator)
    listener.run()
"""
...
# File paths for logging messages
...
class MllpListener:
    ...
    and assigns tasks to handle the received messages.
    """
    ...
    def __init__(self, mllp_address: str, parser: HL7Parser, data_operator: DataOperator):
        ...
        self.running = True  # Flag to control the listener loop
        
        # Ensure directory exists for logging; if not, disable message logging.
        if not os.path.isdir(directory_path):
            logging.error(f"mllp_listener.py: Directory {directory_path} does not exist. Message logging disabled.")
            self.can_log_messages = False
        else:
            logging.info("mllp_listener.py: Message logging enabled.")
    ...
    def open_connection(self):
        ...
        while self.running:
            ...
            try:
                ...
                logging.error(f"mllp_listener.py: [-] Could not connect to {mllp_host}:{mllp_port} ({e}), retrying in 5s...")
                time.sleep(5)
    ...
    def shutdown(self):
        ...
        if self.client_socket:
            try:
                ...
                logging.error(f"mllp_listener.py: Error closing socket: {e}")
        MLLP_SHUTDOWNS.inc()
        logging.info(f"mllp_listener.py: [*] Shutting down MLLP Listener. Total shutdowns: {MLLP_SHUTDOWNS._value.get()}")
        # Do not call exit(); allow the main loop to complete gracefully.
    ...
    def record_messages(self):
        """Writes failed and parsed messages to their respective CSV log files."""
        if not self.can_log_messages:
            logging.warning("mllp_listener.py: Message logging is disabled.")
            return
        try:
            if FAILED_MESSAGES:
                ...
                FAILED_MESSAGES.clear()  # Clear after logging
        except Exception as e:
        ...
        try:
            if PARSED_MESSAGES:
                ...
                PARSED_MESSAGES.clear()  # Clear after logging
        except Exception as e:
            logging.error(f"mllp_listener.py: Error writing to parsed messages file: {e}")
    
    def run(self):
        """
        Continuously listens for HL7 messages, processes them, and sends an acknowledgment (ACK).
        """
        buffer = b""
        try:
            while self.running:
                ...
                # Process complete HL7 messages from the buffer
                while START_BLOCK in buffer and END_BLOCK in buffer:
                    ...
                    if parsed_message is None or parsed_message[0] is None:
                        ...
                        continue  # Continue processing further messages
                    ...
                    try:
                        ...
                        FAILED_MESSAGES.append(parsed_message)
                        self.record_messages()
                        logging.error(f"mllp_listener.py: Error processing message: {e}")
        except socket.timeout:
            logging.warning("mllp_listener.py: [-] Read timeout. Closing connection.")
            self.shutdown()
        except Exception as e:
            logging.error(f"mllp_listener.py: Unexpected error: {e}")
            self.shutdown()
