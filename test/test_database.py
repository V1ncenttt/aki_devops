...
class TestDatabase(unittest.TestCase):
    ...
    def test_get_data(self):
        ...
        self.assertEqual(
            self.datetime_array_to_str(patient_row["creatinine_date_4"].to_numpy()),
            "20240206144500",
        )
    ...
    def test_add_measurement(self):
        ...
        self.assertEqual(
            self.datetime_array_to_str(patient_row["creatinine_date_0"].to_numpy()),
            "20230101120000",
        )
        ...
        self.assertEqual(
            self.datetime_array_to_str(patient_row["creatinine_date_1"].to_numpy()),
            "20230101120005",
        )

    def test_get_data_nonexistent(self):
        """Test getting data for a patient that does not exist."""
        patient_row = self.db.get_data(128)
        columns = self.db.df.columns
        self.assertTrue(pd.DataFrame(columns=columns).equals(patient_row))
